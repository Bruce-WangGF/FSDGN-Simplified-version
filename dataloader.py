...
def populate_train_list(orig_images_path, hazy_images_path):
    ...
    train_list = []
    val_list = []
    # 扫描这个下面的jpg文件，得到的数据类型：'data/image\\NYU2_1.jpg'
    ...
    for image in image_list_haze:
        ...
        image = image.split("/")[-1]
        # print(image)
        # = image.split("_")[0] + "_" + image.split("_")[1] + ".jpg"
...
class dehazing_loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if flip_channel != 0:
        #     data_orig = np.flip(data_orig, 2)
        #     data_hazy = np.flip(data_hazy, 2)
        # # randomly rotation
        # rotation_degree = random.randint(0, 3)
        # data_orig = np.rot90(data_orig, rotation_degree, (1, 2))
        # data_hazy = np.rot90(data_hazy, rotation_degree, (1, 2))
        #
        # data_orig = torch.from_numpy(data_orig.copy())
        # data_hazy = torch.from_numpy(data_hazy.copy())
        # data_orig = torch.from_numpy(np.ascontiguousarray(data_orig)).float()
        # data_hazy = torch.from_numpy(np.ascontiguousarray(data_hazy)).float()
