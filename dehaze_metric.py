...
def dehaze_image(image_path,i):
    ...
    dehaze_net.load_state_dict(torch.load('snapshotsAod/Epoch'+str(i)+'.pth'))

    _,clean_image = dehaze_net(data_hazy)
    ...
    torchvision.utils.save_image(clean_image, "D:/dataset/res/" + image_path.split("/")[-1])
...
if __name__ == '__main__':
    ...
    for i in range(44, 45):
        test_list = glob.glob("D:/dataset/NH/hazy/*")
        #print(test_list)
        ...
        for j in range(1, 6):
            # 复原图
            print(j)
            ref = Image.open('D:/dataset/res/hazy/WGF_' + str(j) + '.png').convert('L')
            ref = ref.resize((256, 256), Image.ANTIALIAS)
            # 原图
            target = Image.open('D:/dataset/NH/gt512/WGF_' + str(j) + '.PNG').convert('L')
